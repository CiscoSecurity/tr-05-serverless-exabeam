...
class HorizontalBarTile(AbstractTile):
    @property
    def _periods(self):
        return ['last_7_days']
    ...
    def _value(self, key, value):
        return {
            ...
            'link_uri': source_uri(
                ...
                current_app.config['URL_PARAMS_FOR_TILE'].replece('now-30d', 'now-1d')
            )
        }
    ...
    def _data(self, visualize_data):
        ...
        for bucket in visualize_data[self._id]['buckets']:
            date = datetime.fromtimestamp(bucket['key']/10**3, timezone.utc).strftime('%b/%d')
            values = []
            value = 0
            for inner_bucket in bucket:
